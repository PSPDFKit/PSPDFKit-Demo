...
- (void)updateDocumentWithMutatingFiles:(void (^)(NSMutableArray *files))fileMutationBlock {
    ...
    [PSPDFKit.sharedInstance.cache removeCacheForDocument:self.rightDocument deleteDocument:NO error:NULL];
...
- (void)saveDocument {
    ...
    [PSPDFProcessor.defaultProcessor generatePDFFromDocument:self.rightDocument pageRanges:@[[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, self.rightDocument.pageCount)]] outputFileURL:savedDocumentURL options:@{PSPDFProcessorAnnotationAsDictionaryKey : @YES, PSPDFProcessorAnnotationTypesKey : @(PSPDFAnnotationTypeAll)} progressBlock:NULL error:NULL];
...
- (PSPDFDocument *)splitDocumentIfRequired:(PSPDFDocument *)document saveAnnotationsInsidePDF:(BOOL)saveAnnotationsInsidePDF {
    if (document.isValid && document.files.count != document.pageCount) {
        ...
        for (NSUInteger pageIndex = 0; pageIndex < document.pageCount; pageIndex++) {
            ...
            NSDictionary *options = saveAnnotationsInsidePDF ? @{PSPDFProcessorAnnotationAsDictionaryKey : @YES, PSPDFProcessorAnnotationTypesKey : @(PSPDFAnnotationTypeAll)} : NULL;
