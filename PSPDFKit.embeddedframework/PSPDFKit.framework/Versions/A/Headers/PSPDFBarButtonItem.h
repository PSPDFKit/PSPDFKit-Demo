...
/**
 Custom subclass that handles an `UIBarButtonItem` within the `UINavigationBar` of `PSPDFViewController`.

 The toolbar system in `PSPDFViewController` is designed to work with both stock `UIBarButtonItem` and `PSPDFBarButtonItem`; but if you want to use `additionalRightBarButtonItems` (where the items are listed inside an `UIActionSheet`) you must use a subclass of `PSPDFBarButtonItem`.

 PSPDFBarButtonItem also gives you access to the `pdfController` and ways to dynamically enable/disable your icon.

 Call `updateBarButtonItem:animated:` if you need to change the `image`/`customView`/`systemImage` after the barButton has been displayed.

 @warning Do not change `target`/`selector` - if the case of `UIActionSheet`/moreButton we call target/selector of the selected `barButtonItem` but with the sender argument of the `PSPDFMoreBarButtonItem`. This is needed to get the correct coordinates in case an `UIPopoverController` follows (which will originate from that `moreBarButtonItem`). If you override target/selector to something generic, you'll never know what button has been selected.
 */
